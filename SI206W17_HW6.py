...
def personal_map(funct_obj, lst1):
    new_list = []
    for item in lst1:
        new_list.append(funct_obj(item))
    return new_list
...
names_with_not_too_much_seniority = [z.name for z in filter(lambda x: len(x.name) > x.years_UM, programmers)]
...
def main_filterer(filenames_lst2):
    lst1 = []
    x = readfiles(filenames_lst2)
    y = len_check(x)
    lst1.extend(y)
    return lst1
